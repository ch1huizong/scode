...
class Slot(object): 
    ...
    def is_idle(self):
        return not (self.queue or self.active)  # queue空且active空
...
class Scraper(object):
    ...
    def open_spider(self, spider):
        ...
        self.slot = Slot()
        yield self.itemproc.open_spider(spider)  # 激发itemproc类
    ...
    def _check_if_closing(self, spider, slot):
        if slot.closing and slot.is_idle():
            slot.closing.callback(spider)  # 如果正好就开启
    ...
    def _process_spidermw_output(self, output, request, response, spider):
        ...
        if isinstance(output, Request):
            ...
            dfd = self.itemproc.process_item(output, spider)  # 处理后的最终结果
