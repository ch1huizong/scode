...
class ImagesPipeline(FilesPipeline):
    ...
    def __init__(self, store_uri, download_func=None, settings=None):
        ...
        if isinstance(settings, dict) or settings is None:
            settings = Settings(settings) # 得到settings实例

        # 先从外部获取，外部没有设置则用类默认设置
    ...
    def from_settings(cls, settings): 
        s3store = cls.STORE_SCHEMES['s3'] # 查找并设置存储
        ...
        store_uri = settings['IMAGES_STORE'] # 存储地点
    ...
    def image_downloaded(self, response, request, info): 
        ...
        for path, image, buf in self.get_images(response, request, info):
            ...
            self.store.persist_file(  # 存储
                path, buf, info,
                meta={'width': width, 'height': height},
                headers={'Content-Type': 'image/jpeg'})
        return checksum

    def get_images(self, response, request, info): # 复数，因为还会存储缩略图
        path = self.file_path(request, response=response, info=info)
        orig_image = Image.open(BytesIO(response.body))

        width, height = orig_image.size
        if width < self.min_width or height < self.min_height: # 太小丢弃
    ...
    def convert_image(self, image, size=None): # Pillow对图片进行操作
    ...
    def get_media_requests(self, item, info): # 可以覆盖
    ...
    def file_path(self, request, response=None, info=None):
        ...
        if not isinstance(request, Request): # 老的，会用file_key或者imge_key
            _warn()
            url = request
        else:
            url = request.url
        ...
        return 'full/%s.jpg' % (image_guid)  # 图片路径
