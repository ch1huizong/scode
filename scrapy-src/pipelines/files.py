...
from scrapy.utils.boto import is_botocore # 跟aws有关了吧
...
class FSFilesStore(object):
    def __init__(self, basedir):
        if '://' in basedir:
            basedir = basedir.split('://', 1)[1]  # 情况file?
    ...
    def persist_file(self, path, buf, info, meta=None, headers=None):
        ...
        with open(absolute_path, 'wb') as f:
            f.write(buf.getvalue()) # 写文件

    def stat_file(self, path, info): # 文件统计信息
        absolute_path = self._get_filesystem_path(path)
        try:
...
class S3FilesStore(object):
    ...
    def __init__(self, uri):
        self.is_botocore = is_botocore()  # 如果有botocore
        if self.is_botocore:
        ...
        self.bucket, self.prefix = uri[5:].split('/', 1)  # 存储桶和子目录
    ...
    def _get_boto_bucket(self): # boto的客户端
    ...
    def _get_boto_key(self, path): # 获取文件, botocore和boto api不同
    ...
    def persist_file(self, path, buf, info, meta=None, headers=None):
        ...
        if self.is_botocore:
            ...
            return threads.deferToThread(  # 这里才上传
...
class GCSFilesStore(object):
    ...
    def __init__(self, uri):
        ...
        self.bucket = client.bucket(bucket) # google bucket
        self.prefix = prefix
...
class FilesPipeline(MediaPipeline):
    ...
    def __init__(self, store_uri, download_func=None, settings=None):
        ...
        self.store = self._get_store(store_uri)  # 返回存储类实例
    ...
    def item_completed(self, results, item, info): # 单个item的所有文件
