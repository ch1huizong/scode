...
############################################################
#
#   第一部分，主要是解析App的，包括第一步建立wsgi 处理环境
#  
############################################################
...
class BaseHandler:
    ...
    def setup_environ(self):
        ...
        env = self.environ = self.os_environ.copy()  # 总体环境，包含操作系统环境
        self.add_cgi_vars()                          # CGI环境加进去
    ...
    def finish_response(self):
        ...
        """
        # 异步服务器如何处理？建立回调函数在事件循环中？
        try:
            if not self.result_is_file() or not self.sendfile():
                for data in self.result:
                    self.write(data)       #迭代数据
                self.finish_content()
        finally:
            self.close()
    ...
    def set_content_length(self):
        ...
        try:
            ...
            if blocks==1:      # 若block就一块，可以设置content-type,如果好多块呢?
                               # 功能有限？
                self.headers['Content-Length'] = str(self.bytes_sent) #引用bytes_sent?
                return
    ...
    def cleanup_headers(self):
        ...
        """
        if 'Content-Length' not in self.headers: #如果app没有设置Content-length信息
    ...
    def write(self, data):
        ...
        if not self.status:
            ...
            self.send_headers()            #第一次发送时，先发送头信息
        else:
    ...
    def send_headers(self):
        ...
        self.headers_sent = True   # 捕捉到这个，说明头已经发送了
    ...
    def log_exception(self,exc_info):    #记录错误
    ...
    def handle_error(self):    #处理错误,很重要
        """Log current error, and send error output to client if possible"""
        self.log_exception(sys.exc_info())
        if not self.headers_sent:  # 若头还没发送时处理
...
class SimpleHandler(BaseHandler):
    ...
    def _write(self,data):
        ...
        self._write = self.stdout.write  #为什么？留着
